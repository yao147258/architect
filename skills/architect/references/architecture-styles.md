...
通知库存、积分、推荐、通知等多个模块）——同一事件的下游消费方数量通常 ≥3 个，少于 3 个时进程内直接调用/同步调用更简单，不必引入消息中间件；下游可以接受最终一致，延迟容忍窗口通常在秒级到分钟级。
...
**适用前提：** 负载波峰波谷差异大——峰谷流量比通常 ≥5:1，或日均资源利用率 <20%；团队规模通常 ≤5 人，不想负担
运维成本；单次请求执行时间 <15 分钟（多数 FaaS 平台的执行时长上限）。
